...
def main():
    ...
    if len(sys.argv)>1:
        ...
        catalogue = 'sm_9.5_s0.2_sfr_c-0.8_250_Chinchilla_250'
        sm_low=9.5
        sm_high=9.8
    ...
    savepath = cu.get_output_path() + 'analysis/central_quenching/observables/'
    filename_1 = catalogue+'_wp_all_'+str(sm_low)+'_'+str(sm_high)+'.dat'
    ascii.write(data_1, savepath+filename_1)
    filename_2 = catalogue+'_wp_q_'+str(sm_low)+'_'+str(sm_high)+'.dat'
    ascii.write(data_2, savepath+filename_2)
    filename_3 = catalogue+'_wp_sf_'+str(sm_low)+'_'+str(sm_high)+'.dat'
    ascii.write(data_3, savepath+filename_3)
    
    plt.figure()
    plt.plot(rp_bin_centers, result_all*rp_bin_centers, color='black')
    plt.plot(rp_bin_centers, result_q*rp_bin_centers, color='red')
    plt.plot(rp_bin_centers, result_sf*rp_bin_centers, color='blue')
    plt.xscale('log')
    plt.yscale('log')
    plt.show()
