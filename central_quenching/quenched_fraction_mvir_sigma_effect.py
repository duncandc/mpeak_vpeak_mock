...
def main():
    ...
    rho = -1.0
    sigmas = np.array([0.0,0.1,0.2,0.3])
    ...
    for i,sigma in enumerate(sigmas):
        ...
        pa_1 = cu.plotting.colorline(10**bin_centers, f_red_cen_halo, sigma, axes[0],\
                                     vmin=np.min(sigmas), vmax=np.max(sigmas), cmap='cool')
        pb_1 = cu.plotting.colorline(10**bin_centers, f_red_cen_am, sigma, axes[1],\
                                     vmin=np.min(sigmas), vmax=np.max(sigmas), cmap='cool')
        pa.append(pa_1)
        pb.append(pb_1)
    ...
    axes[0].legend(pa,sigmas,loc=4,fontsize=10,frameon=False,labelspacing=0.01,title=r'$\sigma_{\rm SMHM}$')
    axes[0].text(10.0**11.1,0.9,"intrinisic")
    axes[0].text(10.0**11.1,0.84,r'$\rho_{\rm SSFR}=$'+str(rho))
    
    #axes[1].set_xlabel(r'$M_{\rm AM}$ $[h^{-1}M_{\odot}]$')
    ...
    axes[1].legend(pb,sigmas,loc=4,fontsize=10,frameon=False,labelspacing=0.01,title=r'$\sigma_{\rm SMHM}$')
    axes[1].text(10.0**11.2,0.85,"inverse abundance \n matching")
    axes[1].text(10.0**11.2,0.78,r'$\rho_{\rm SSFR}=$'+str(rho))
    plt.show()
    savepath = cu.get_plot_path()+'/analysis/central_quenching/'
    filename = 'f_q_cen_compare_sigma'
